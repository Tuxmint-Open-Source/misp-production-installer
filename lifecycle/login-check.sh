...
Usage:
  ./lifecycle/login-check.sh [options]

Options:
  --install-dir PATH   Deployment directory (default: /opt/misp-docker)
  --url URL            Override BASE_URL from .env
  --strict-tls         Verify TLS certificates (default; retained for compatibility)
  --insecure           Explicitly allow HTTP or unverified HTTPS (unsafe for credentials)
...
TLS certificate verification is enabled by default. The script reads
ADMIN_EMAIL and ADMIN_PASSWORD from .env, fetches the login
...
python3 - "$ENV_FILE" "$URL_OVERRIDE" "$INSECURE" "$MACHINE_READABLE" <<'PY'
...
def print_machine(result):
    for key in [
        'status', 'reason', 'transport_security', 'login_form_status', 'login_post_status', 'final_path',
        'invalid_credentials_marker', 'csrf_marker', 'server_error_marker',
        'authenticated_session_marker', 'login_success', 'logout_attempted', 'logout_status', 'logout_final_path'
    ]:
        print(f'{key}={result[key]}')

def failure_result(reason, transport_security, form_status='not-run', post_status='not-run', final_path='not-run'):
    return {
        'status': 'failed', 'reason': reason, 'transport_security': transport_security,
        'login_form_status': form_status, 'login_post_status': post_status, 'final_path': final_path,
        'invalid_credentials_marker': 'false', 'csrf_marker': 'false', 'server_error_marker': 'false',
        'authenticated_session_marker': 'false', 'login_success': 'false',
        'logout_attempted': 'false', 'logout_status': 'not-run', 'logout_final_path': 'not-run',
    }

def fail(reason, message, transport_security='not-established', form_status='not-run', post_status='not-run', final_path='not-run'):
    result = failure_result(reason, transport_security, form_status, post_status, final_path)
    if machine_readable:
...
def print_human(result, base_url, email):
    print(f'Transport security: {result["transport_security"]}')
    ...
    elif result['reason'] == 'no-authenticated-session-marker':
        print('- The response did not contain positive evidence of an authenticated MISP session.')
    else:
    ...
    print('- If installation already completed, re-run: ./lifecycle/doctor.sh --install-dir /opt/misp-docker')
    print('- Verify the generated account with: ./lifecycle/admin-credentials.sh --install-dir /opt/misp-docker')
...
env_file, url_override, insecure, machine_readable = sys.argv[1], sys.argv[2], sys.argv[3] == 'true', sys.argv[4] == 'true'
...
if not base_url:
    fail('configuration-error', 'BASE_URL is missing in .env and --url was not provided')
if not email:
    fail('configuration-error', 'ADMIN_EMAIL is missing in .env')
if not password:
    fail('configuration-error', 'ADMIN_PASSWORD is missing in .env')

parsed_base = urlparse(base_url)
if parsed_base.scheme not in {'http', 'https'} or not parsed_base.netloc:
    fail('configuration-error', 'BASE_URL must be an absolute HTTP or HTTPS URL')
if parsed_base.username is not None or parsed_base.password is not None:
    fail('configuration-error', 'BASE_URL must not contain embedded credentials')
if parsed_base.scheme != 'https' and not insecure:
    fail('transport-refused', 'Refusing to send administrator credentials over unencrypted HTTP; use HTTPS or explicitly pass --insecure')
if insecure:
    print('WARNING: TLS verification is disabled or HTTP is allowed; administrator credentials are not protected against interception.', file=sys.stderr)
ctx = ssl._create_unverified_context() if insecure else ssl.create_default_context()
opener = build_opener(SameOriginRedirectHandler(), HTTPCookieProcessor(CookieJar()), HTTPSHandler(context=ctx))
...
try:
    ...
    reason = 'cross-origin-redirect' if 'cross-origin redirect refused' in str(exc) else 'login-form-http-error'
    fail(reason, str(exc), 'insecure-explicit' if insecure else 'verified-tls')
except URLError as exc:
    reason = 'tls-verification-failed' if isinstance(exc.reason, ssl.SSLCertVerificationError) else 'transport-error'
    fail(reason, exc.__class__.__name__, 'insecure-explicit' if insecure else 'verified-tls')

if origin(get_final_url) != origin(login_url):
    fail('cross-origin-redirect', 'Login form redirected to a different origin; refusing to send administrator credentials', 'insecure-explicit' if insecure else 'verified-tls', str(get_status))
...
try:
    ...
    reason = 'cross-origin-redirect' if 'cross-origin redirect refused' in str(exc) else 'login-post-http-error'
    fail(reason, str(exc), 'insecure-explicit' if insecure else 'verified-tls', str(get_status), str(exc.code))
except URLError as exc:
    reason = 'tls-verification-failed' if isinstance(exc.reason, ssl.SSLCertVerificationError) else 'transport-error'
    fail(reason, exc.__class__.__name__, 'insecure-explicit' if insecure else 'verified-tls', str(get_status))

if origin(final_url) != origin(login_url):
    fail('cross-origin-redirect', 'Login response redirected to a different origin; refusing to trust the session result', 'insecure-explicit' if insecure else 'verified-tls', str(get_status), str(post_status))
...
session_parser = LoginFormParser()
session_parser.feed(body)
authenticated = any(
    origin(urljoin(final_url, href)) == origin(login_url)
    and urlparse(urljoin(final_url, href)).path.rstrip('/') == '/users/logout'
    for href in session_parser.links
)
success = 200 <= post_status < 400 and authenticated and not invalid and not csrf and not server_error and not still_login
...
if not success:
    if invalid:
        ...
        reason = 'no-authenticated-session-marker'
    else:
        reason = 'unknown'
...
result = {
    ...
    'reason': reason,
    'transport_security': 'insecure-explicit' if insecure else 'verified-tls',
    ...
    'authenticated_session_marker': str(authenticated).lower(),
