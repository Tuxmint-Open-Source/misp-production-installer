...
> [!IMPORTANT]
> `v1.3.1` is pending full validation as an immutable tag and published operator-bundle artifact. `v1.3.0` remains the latest validated-compatible release, and `v1.2.0` remains retained historical validated evidence. PR-branch evidence remains capability evidence only and is not a substitute for exact-tag validation.
>
