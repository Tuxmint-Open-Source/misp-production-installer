...
> [!IMPORTANT]
> `v1.3.1` is the latest validated-compatible release as an immutable tag and published operator-bundle artifact. `v1.3.0` and `v1.2.0` remain retained historical validated evidence. PR-branch evidence remains capability evidence only and is not a substitute for exact-tag validation.
>
