...
This page defines the public support scope of the stable release line. `v1.3.1` is the latest published release; exact-tag validation is pending, so `v1.3.0` remains the latest validated-compatible release.
...
| Lifecycle helpers | install, doctor, login check, healthcheck, anonymous SOS report, backup, restore, update, reset dry-run, restore-based rollback, no-lock-in Compose usage | Exact `v1.3.1` compatibility is pending validation; exact `v1.3.0` compatibility is validated only for the component tuple listed in the compatibility matrix. |
