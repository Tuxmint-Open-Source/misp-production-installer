...
> [!IMPORTANT]
> `v1.2.0` is the latest published release and is pending exact-tag validation; `v1.1.0` remains the latest validated release for the documented component set and single-server Docker lifecycle-manager scope. Validate your own deployment assumptions and keep backups before relying on a deployment operationally.
