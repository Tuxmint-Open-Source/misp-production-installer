...
class StaticRepoTests(unittest.TestCase):
    ...
    def test_release_candidate_is_pending_but_previous_rc_remains_validated(self):
        version = (ROOT / 'VERSION').read_text().strip()
        readme = (ROOT / 'README.md').read_text()
        compatibility = (ROOT / 'docs' / 'compatibility.md').read_text()
        matrix = (ROOT / 'docs' / 'validation' / 'matrix.md').read_text()
        readiness = (ROOT / 'docs' / 'production-readiness.md').read_text()
        rc2_report = (ROOT / 'docs' / 'validation' / 'compatibility-v1.0.0-rc.2-misp-core-v2.5.43.md').read_text()
        versioning = (ROOT / 'docs' / 'versioning.md').read_text()
        ...
        self.assertIn('Pending exact-tag validation after tag publication', compatibility)
        ...
        self.assertIn('`v1.0.0-rc.3` pending exact-tag validation', readiness)
        self.assertIn('| `v1.0.0-rc.3` | Current release candidate pending exact-tag validation', security)
        self.assertIn('Overall result | ✅ Validated compatible', rc2_report)
        self.assertIn('final `v1.0.0` release must still be tagged and validated separately', rc2_report)
