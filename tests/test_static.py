...
class StaticRepoTests(unittest.TestCase):
    ...
    def test_documentation_red_line_entry_points_exist(self):
        ...
        self.assertIn('git checkout v1.0.0-rc.3', getting_started)
    ...
    def test_release_candidate_is_validated_but_final_v1_remains_pending(self):
        ...
        matrix = (ROOT / 'docs' / 'validation' / 'matrix.md').read_text()
        readiness = (ROOT / 'docs' / 'production-readiness.md').read_text()
        rc3_report = (ROOT / 'docs' / 'validation' / 'compatibility-v1.0.0-rc.3-misp-core-v2.5.43.md').read_text()
        versioning = (ROOT / 'docs' / 'versioning.md').read_text()
        ...
        self.assertIn('Current manager version: `1.0.0-rc.3`', readme)
        self.assertIn('git checkout v1.0.0-rc.3', readme)
        self.assertIn('`v1.0.0-rc.3` release candidate tag', readme)
        self.assertIn('✅ Validated compatible', readme)
        self.assertIn('`v1.0.0-rc.3` release candidate tag', compatibility)
        self.assertIn('compatibility-v1.0.0-rc.3-misp-core-v2.5.43.md', compatibility)
        self.assertIn('`v1.0.0-rc.2` release candidate tag', compatibility)
        self.assertIn('✅ Validated compatible', compatibility)
        self.assertIn('`v1.0.0-rc.3` release candidate tag', matrix)
        self.assertIn('compatibility-v1.0.0-rc.3-misp-core-v2.5.43.md', matrix)
        self.assertIn('`v1.0.0-rc.3` validated compatible', readiness)
        self.assertIn('| `v1.0.0-rc.3` | Current release candidate pending exact-tag validation', security)
        self.assertIn('Overall result | ✅ Validated compatible', rc3_report)
        self.assertIn('Total duration | 2312 seconds', rc3_report)
        self.assertIn('final `v1.0.0` release must still be tagged and validated separately', rc3_report)
