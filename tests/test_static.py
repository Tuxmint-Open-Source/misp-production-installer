...
class StaticRepoTests(unittest.TestCase):
    ...
    def test_always_running_repository_gate_is_read_only_and_complete(self):
        ...
        self.assertIn('git diff --check "$PUSH_BEFORE_SHA..$CURRENT_SHA"', workflow)
        self.assertIn('git diff-tree --root --check --no-commit-id -r "$CURRENT_SHA"', workflow)
        self.assertIn('git diff --check "$CURRENT_SHA^..$CURRENT_SHA"', workflow)
        self.assertIn('Unsupported event for repository diff gate', workflow)
