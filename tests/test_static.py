...
class StaticRepoTests(unittest.TestCase):
    ...
    def test_final_v1_release_is_pending_exact_tag_validation(self):
        version = (ROOT / 'VERSION').read_text().strip()
        readme = (ROOT / 'README.md').read_text()
        ...
        self.assertIn('`v1.0.0` pending exact-tag validation', readiness)
        self.assertIn('| `v1.0.0` | Current stable release line pending exact-tag compatibility validation', security)
